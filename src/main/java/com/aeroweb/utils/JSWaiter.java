...
public class JSWaiter {
    ...
    public void setDriver () {
        jsWait = new WebDriverWait(DriverUtil.getInstance().getDriver(), 10);
        jsExec = (JavascriptExecutor) DriverUtil.getInstance().getDriver();
    }
    ...
    private void waitForJQueryLoad() {
        try {
            ExpectedCondition<Boolean> jQueryLoad = driver -> ((Long) ((JavascriptExecutor) DriverUtil.getInstance())
    ...
    private void waitUntilJSReady() {
        try {
            ExpectedCondition<Boolean> jsLoad = driver -> ((JavascriptExecutor) DriverUtil.getInstance())
 ...
 //       new DriverUtil().explicitWait(.5);
    ...
    public void waitForElementAreComplete(By by, int expected) {
        ExpectedCondition<Boolean> angularLoad = driver -> {
            int loadingElements = common.getElements(by).size();
    ...
    public void waitForAnimationToComplete(String css) {
        ExpectedCondition<Boolean> angularLoad = driver -> {
            int loadingElements = common.getElements(By.cssSelector(css)).size();
            return loadingElements == 0;
        };
        jsWait.until(angularLoad);
    }
